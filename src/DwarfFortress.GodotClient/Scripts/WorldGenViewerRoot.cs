...
public partial class WorldGenViewerRoot : Node2D
{
    ...
    private static Color CreatureSpawnColor(string creatureDefId) => creatureDefId switch
    {
        CreatureDefIds.Cat => new Color(0.96f, 0.86f, 0.34f),
        CreatureDefIds.Dog => new Color(0.86f, 0.56f, 0.28f),
        CreatureDefIds.Elk => new Color(0.66f, 0.42f, 0.22f),
        CreatureDefIds.GiantCarp => new Color(0.22f, 0.70f, 0.98f),
        CreatureDefIds.Goblin => new Color(0.46f, 0.90f, 0.34f),
        CreatureDefIds.Troll => new Color(0.74f, 0.32f, 0.86f),
    ...
    private GeneratedWorldHistory? BuildCurrentHistoryForRegionGeneration()
    {
        ...
        var roads = snapshot.Roads;

        var civs = snapshot.Civilizations.Select(civ => new CivilizationRecord
        {
            Id = civ.CivilizationId,
            Name = civ.Name,
            Capital = default,
            Territory = [],
            Influence = 0f,
        ...
        var sites = snapshot.Sites.Select(site => new SiteRecord
        {
            Id = site.SiteId,
            Name = site.Name,
            Kind = site.Kind,
        ...
        return new GeneratedWorldHistory
        {
            ...
            Sites = sites,
            Roads = roads,
            TerritoryByTile = snapshot.TerritoryByTile,
        };
    }
    ...
    private void RefreshHistoryFeed()
    {
        ...
        sb.AppendLine($"Civilizations: {snapshot.Civilizations.Count} | Sites: {snapshot.Sites.Count} | Roads: {snapshot.Roads.Count}");
        sb.AppendLine();
        ...
        sb.AppendLine("- Character-level history is not modeled yet (events/civs/sites are live).");
