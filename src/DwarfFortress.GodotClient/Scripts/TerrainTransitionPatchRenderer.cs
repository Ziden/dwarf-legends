...
public static class TerrainTransitionPatchRenderer
{
        ...
        bool trimEnd)
    {
        ...
        var variant = (byte)Math.Clamp((int)(variantNoise * EdgePatchVariantCount), 0, EdgePatchVariantCount - 1);
        var patchTexture = GetGroundEdgePatchTexture(neighborGround, direction, variant, trimStart, trimEnd);
        ...
        TerrainCornerQuadrant corner)
    {
        ...
        var variant = (byte)Math.Clamp((int)(variantNoise * CornerPatchVariantCount), 0, CornerPatchVariantCount - 1);
        var patchTexture = GetGroundCornerPatchTexture(cornerGround, corner, variant);
        canvas.DrawTextureRect(patchTexture, rect, false);
    }

    private static Texture2D GetGroundEdgePatchTexture(
        GroundVisualData neighborGround,
        TerrainEdgeDirection direction,
        byte variant,
        bool trimStart,
    ...
    private static Texture2D GetGroundCornerPatchTexture(
        GroundVisualData cornerGround,
        TerrainCornerQuadrant corner,
        byte variant)
    {
